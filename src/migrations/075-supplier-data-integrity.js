...
module.exports = {
  ...
  async up(sequelize) {
    ...
    await this.upsertSupplier(sequelize, belicaData);
    ...
    await this.upsertSupplier(sequelize, fiorillaData);
    ...
    await this.upsertSupplier(sequelize, oilGuyData);
    ...
    await this.upsertSupplier(sequelize, omniData);
  ...
  /**
   * Upsert a supplier: update existing ScrapeConfigSync record or insert new.
   * Matches by website domain. Uses ON CONFLICT (slug) for idempotency.
   */
  async upsertSupplier(sequelize, supplier) {
    // First try to find existing record by website domain
    ...
    if (existing) {
      ...
      `, {
        bind: [
          ...
          supplier.website, supplier.addressLine1, supplier.city, supplier.state,
