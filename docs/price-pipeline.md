...
- **Cross-fuel identity guard (heatingoil-u2gr)**: a secondary-fuel price (kerosene/propane) IDENTICAL to the supplier's primary heating-oil price is rejected as a likely mislabeled capture (e.g. a loose `fuels.propane` regex grabbing the oil price — dolanoilservice shipped $4.899 oil under propane). The per-fuel FUEL_PRICE_RANGES overlap (oil [2,6] ∩ kerosene [2.5,7] ∩ propane [1.5,5] = [2.5,5]), so a leaked value sits in-range and the range gates miss it — value-identity is the catch. Lives in `evaluatePriceSanity` via the optional `primaryPrice` arg; both scrape paths pass `result.pricePerGallon` ONLY when validating a secondary fuel (never for the primary oil, so oil is never rejected against itself; skipped when no oil price exists, e.g. primary_fuel_optional suppliers). Reason prefix `cross-fuel:`. Same `DISABLE_PRICE_SANITY=true` kill switch. NOTE: this only catches secondary==oil duplicate captures; a leaked secondary value that DIFFERS from oil still needs label/context anchoring (follow-up).
