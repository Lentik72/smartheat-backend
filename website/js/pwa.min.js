(function(){"serviceWorker"in navigator&&navigator.serviceWorker.register("/sw.js").catch(()=>{});let t=null;if(!/Android/i.test(navigator.userAgent)||window.matchMedia("(display-mode: standalone)").matches)return;function r(){const e=localStorage.getItem("pwa-banner-dismissed");if(!e)return!1;const a=parseInt(e,10),c=10080*60*1e3;return Date.now()-a<c}function l(){return parseInt(localStorage.getItem("pwa-visit-count")||"0",10)}function d(){let e=l()+1;return localStorage.setItem("pwa-visit-count",e.toString()),e}window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),t=e,typeof gtag=="function"&&gtag("event","pwa_prompt_ready",{platform:"android"})});function n(){if(!t||r()||document.getElementById("pwa-install-banner"))return;const e=document.createElement("div");e.id="pwa-install-banner",e.innerHTML=`
